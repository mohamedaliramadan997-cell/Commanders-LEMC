...
import { titleCase, ridingExperienceOptionsHtml, yearsToSinceYear, sinceYearToYears } from "./utils.js";
...
const FIELDS = [
  ...
  // riding_since_year is handled separately below (rendered as a live years-of-experience dropdown)
...
function openEdit(id) {
  ...
  formEl.innerHTML = FIELDS.map(([key, label, type, span, placeholder]) => {
    ...
    if (type.startsWith("select:")) {
      ...
      input = `<input data-key="${key}" type="${type}" value="${escapeHtml(val)}" ${placeholder ? `placeholder="${escapeHtml(placeholder)}"` : ""} />`;
    }
    return `<div class="${span === "full" ? "full" : ""}"><label>${label}</label>${input}</div>`;
  }).join("") + `
    <div><label>Riding Experience (auto-updates every year)</label>
      <select data-key="riding_since_year" id="riding-years-select">
        ${ridingExperienceOptionsHtml(sinceYearToYears(member.riding_since_year))}
      </select>
    </div>`;
...
async function save() {
  ...
  inputs.forEach((el) => {
    ...
    if (el.dataset.key === "riding_since_year") {
      payload.riding_since_year = v === "" ? null : yearsToSinceYear(v);
      return;
    }
